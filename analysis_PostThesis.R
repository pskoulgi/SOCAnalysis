...
{
  siteNames = data.frame(SITE.ID = c("CT1", "CT2", "CT3", 
                                     "CT4", "CT5", "CT6",
                                     "FR1", "FR2", "FR3",
                                     "FR4", "FR5", "FR6"), 
                         SITE.NAME = c("Kokka1", "Kokka2", "Kutiyalu1", 
                                       "Perumbadi1", "Perumbadi2", "Kutiyalu2",
                                       "Rudraguppe", "Betoli", "Arji",
                                       "Karnad", "Kolthodu", "Arapattu"))
  ...
  cCycle <- left_join(cCycle, siteNames, by="SITE.ID")
  ...
  ggplot(soilcMeans, aes(y=MEAN, x=SITE.TYPE, fill=SITE.TYPE)) + theme_classic() +
    ...
    geom_errorbar(aes(ymin=MEAN-(SE), ymax=MEAN+(SE)), width=.07,
  ...
  soilNLittMeans <- cCycle %>% group_by(SITE.ID) %>% 
    summarise(SITE.TYPE=first(SITE.TYPE),
              ...
              LARGE.TREE.BA.FRAC.MEAN = mean(LARGE.TREE.BA.FRAC),
              LARGE.TREE.BA.FRAC.SE = sd(LARGE.TREE.BA.FRAC)/sqrt(n()),
  ...
  ggplot(filter(soilNLittMeans, SITE.TYPE=="CT"),
    ...
    theme_classic() + 
    geom_point(aes(size=BAS.AREA.MEAN)) +
    # geom_point(aes(size=LARGE.TREE.BA.FRAC.MEAN)) +
    geom_text(aes(label=paste(BAS.AREA.MEAN, "\ ",
                              "(", SITE.NAME, ")", sep=" ")),
              size=5, hjust=0, vjust=0, nudge_x=4, parse=TRUE) +    
    ...
    theme(legend.title=element_text(size = 15),
          ...
          axis.text = element_text(size = 18)) +
    scale_size("Basal area \n (m.sq per ha)")
  #ggsave("figs/SoilCvsLitterWtvsBasArea_SiteAvgs_ContigOnly.png")
  
  ggplot(filter(soilNLittMeans, SITE.TYPE=="CT"),
         aes(y=SIR.MEAN, x=LITTER.WT.MEAN)) + 
    theme_classic() + 
    geom_point(aes(size=BAS.AREA.MEAN)) +
    # geom_point(aes(size=LARGE.TREE.BA.FRAC.MEAN)) +
    geom_text(aes(label=paste(BAS.AREA.MEAN, "\ ",
    ...
    xlab("\n Litter weight (gm per m.sq)") + ylab("Soil SIR \n") +
    theme(legend.title=element_text(size = 15),
          legend.text = element_text(size = 15),
          legend.position = c(0.9, 0.85),
          #legend.background = element_rect(color="grey"),
          ...
          axis.text = element_text(size = 18)) +
    scale_size("Basal area \n (m.sq per ha)")
  #ggsave("figs/SIRvsLitterWtvsBasArea_SiteAvgs_ContigOnly.png")
  
  ggplot(filter(soilNLittMeans, SITE.TYPE=="FR"),
                aes(y=SOIL.C.MEAN, x=LITTER.WT.MEAN)) + 
    theme_classic() + 
    geom_point(aes(size=BAS.AREA.MEAN)) +
    # geom_point(aes(size=LARGE.TREE.BA.FRAC.MEAN)) +
    ...
    theme(legend.title=element_text(size = 15),
          ...
          axis.text = element_text(size = 18)) +
    scale_size("Basal area \n (m.sq per ha)")
  #ggsave("figs/SoilCvsLitterWtvsBasArea_SiteAvgs_FragOnly.png")
  ...
  ggplot(filter(soilNLittMeans, SITE.TYPE=="FR"),
    ...
    theme(legend.title=element_text(size = 15),
          ...
          axis.text = element_text(size = 18)) +
    scale_size("Basal area \n (m.sq per ha)")
  #ggsave("figs/SIRvsLitterWtvsBasArea_SiteAvgs_FragOnly.png")
  ...
  library(lme4)
  S.soilcFragOnlyLMM <- lmer(SOIL.C.S ~ SITE.TYPE + 
                                        (1|SITE.ID),
                             data=cCycle.S, REML=FALSE)
  S.soilcLittPIntFragLMM <- lmer(SOIL.C.S ~ (LITTER.P.S * SITE.TYPE) +
                                            (1|SITE.ID),
                                 data=cCycle.S, REML=FALSE)
  S.soilcLittCNIntFragLMM <- lmer(SOIL.C.S ~ (LITTER.CN.RATIO.S * SITE.TYPE) + 
                                             (1|SITE.ID),
                                  data=cCycle.S, REML=FALSE)
  S.soilcLittWtIntFragLMM <- lmer(SOIL.C.S ~ (LITTER.WT.S * SITE.TYPE) + 
                                             (1|SITE.ID),
                                  data=cCycle.S, REML=FALSE)
  ...
  summary(model.avg(S.soilcLittCNLMM, S.soilcLittCNIntFragLMM,
                    S.soilcLittPLMM, S.soilcLittPIntFragLMM,
                    S.soilcLittWtLMM, S.soilcLittWtIntFragLMM,
                    S.soilcBasAreaLMM, S.soilcBasAreaIntFragLMM,
                    S.soilcFragOnlyLMM))
  r.squaredGLMM(S.soilcLittCNLMM)
  r.squaredGLMM(S.soilcLittCNIntFragLMM)
  r.squaredGLMM(S.soilcLittPLMM)
  r.squaredGLMM(S.soilcLittPIntFragLMM)
  r.squaredGLMM(S.soilcLittWtLMM)
  r.squaredGLMM(S.soilcLittWtIntFragLMM)
  r.squaredGLMM(S.soilcBasAreaLMM)
  r.squaredGLMM(S.soilcBasAreaIntFragLMM)
  r.squaredGLMM(S.soilcFragOnlyLMM)
  ...
  #             Table 2               #
  #####################################
  S.sirFragOnlyLMM <- lmer(SOIL.SIR.S ~ SITE.TYPE +
                                        (1|SITE.ID),
                           data=cCycle.S, REML=FALSE)
  S.sirLittPIntFragLMM <- lmer(SOIL.SIR.S ~ (LITTER.P.S * SITE.TYPE) +
                                            (1|SITE.ID),
                               data=cCycle.S, REML=FALSE)
  S.sirLittCNIntFragLMM <- lmer(SOIL.SIR.S ~ (LITTER.CN.RATIO.S * SITE.TYPE) + 
                                             (1|SITE.ID),
                                data=cCycle.S, REML=FALSE)
  S.sirLittWtIntFragLMM <- lmer(SOIL.SIR.S ~ (LITTER.WT.S * SITE.TYPE) + 
                                             (1|SITE.ID),
                                data=cCycle.S, REML=FALSE)
  ...
  summary(model.avg(S.sirLittCNLMM, S.sirLittCNIntFragLMM,
                    ...
                    S.sirLittWtLMM, S.sirLittWtIntFragLMM,
                    S.sirBasAreaLMM, S.sirBasAreaIntFragLMM,
                    S.sirFragOnlyLMM))
  r.squaredGLMM(S.sirLittCNLMM)
  r.squaredGLMM(S.sirLittPLMM)
  r.squaredGLMM(S.sirLittWtLMM)
  r.squaredGLMM(S.sirBasAreaLMM)
  r.squaredGLMM(S.sirLittCNIntFragLMM)
  r.squaredGLMM(S.sirLittPIntFragLMM)
  r.squaredGLMM(S.sirLittWtIntFragLMM)
  r.squaredGLMM(S.sirBasAreaIntFragLMM)
  r.squaredGLMM(S.sirFragOnlyLMM)
